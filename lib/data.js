...
// 분류 근거: NCS 대분류 19(전기·전자) 반도체개발/제조, 02(경영·회계·사무) 계열의
// 능력단위를 참고해 시연 규모로 축약. 실제 NCS 코드와 1:1 매핑되지는 않는다.
export const SKILLS = [
  // 설계 (전공정 설계 · 검증 · 패키지)
  { id: "verilog", name: "Verilog/RTL 설계", cat: "설계" },
  { id: "verification", name: "설계 검증(UVM)", cat: "설계" },
  { id: "analog", name: "아날로그 회로 설계", cat: "설계" },
  { id: "layout", name: "레이아웃 설계", cat: "설계" },
  { id: "lowpower", name: "저전력 설계", cat: "설계" },
  { id: "dft", name: "DFT/테스트 설계", cat: "설계" },
  { id: "pkg", name: "패키지 설계", cat: "설계" },
  // 공정 (전공정 · 후공정 · 설비)
  { id: "litho", name: "포토/리소그래피", cat: "공정" },
  { id: "etch", name: "식각(Etch) 공정", cat: "공정" },
  { id: "thinfilm", name: "박막(Thin Film)", cat: "공정" },
  { id: "implant", name: "이온주입", cat: "공정" },
  { id: "cmp", name: "CMP(평탄화)", cat: "공정" },
  { id: "metrology", name: "계측·검사(Metrology)", cat: "공정" },
  { id: "equip", name: "설비 엔지니어링(PM)", cat: "공정" },
  { id: "atetest", name: "테스트 프로그램(ATE)", cat: "공정" },
  { id: "yield", name: "수율 분석", cat: "공정" },
  // 품질
  { id: "spc", name: "SPC/공정통계", cat: "품질" },
  { id: "reliability", name: "신뢰성 평가", cat: "품질" },
  { id: "fa", name: "불량 분석(FA)", cat: "품질" },
  { id: "eightd", name: "8D 문제해결", cat: "품질" },
  { id: "custqual", name: "고객 품질 대응", cat: "품질" },
  // 데이터·AI
  { id: "python", name: "Python 데이터분석", cat: "데이터·AI" },
  { id: "sql", name: "SQL·데이터 엔지니어링", cat: "데이터·AI" },
  { id: "ml", name: "머신러닝 모델링", cat: "데이터·AI" },
  { id: "anomaly", name: "시계열 이상탐지", cat: "데이터·AI" },
  ...
  { id: "cost", name: "원가관리", cat: "경영지원" },
  { id: "tax", name: "세무", cat: "경영지원" },
  { id: "erp", name: "ERP(SAP) 운영", cat: "경영지원" },
  { id: "hrm", name: "인사운영(HRM)", cat: "경영지원" },
  { id: "recruit", name: "채용·선발", cat: "경영지원" },
  { id: "hrd", name: "교육·HRD", cat: "경영지원" },
  ...
  { id: "scm", name: "SCM·수요계획", cat: "경영지원" },
  { id: "security", name: "정보보안", cat: "경영지원" },
];

// 스킬 온톨로지: 스킬 간 연관 관계 (그래프 엣지)
export const SKILL_EDGES = [
  // 설계
  ["verilog", "dft"], ["verilog", "layout"], ["verilog", "verification"],
  ["verification", "dft"], ["analog", "layout"], ["verilog", "lowpower"],
  ["lowpower", "layout"], ["pkg", "layout"], ["pkg", "reliability"],
  // 전공정
  ["litho", "etch"], ["etch", "thinfilm"], ["thinfilm", "implant"],
  ["implant", "cmp"], ["cmp", "litho"], ["litho", "metrology"],
  ["metrology", "yield"], ["litho", "yield"], ["equip", "etch"],
  ["equip", "thinfilm"], ["equip", "metrology"],
  // 후공정·테스트
  ["atetest", "dft"], ["atetest", "pkg"], ["atetest", "reliability"],
  // 품질
  ["yield", "spc"], ["spc", "fa"], ["reliability", "fa"],
  ["fa", "eightd"], ["eightd", "custqual"], ["custqual", "reliability"],
  // 데이터·AI
  ["yield", "python"], ["python", "ml"], ["ml", "cv"],
  ["ml", "mlops"], ["python", "bigdata"], ["bigdata", "mlops"],
  ["python", "sql"], ["sql", "bigdata"], ["ml", "anomaly"],
  ["anomaly", "spc"], ["anomaly", "equip"], ["cv", "metrology"],
  ["cv", "fa"], ["spc", "python"], ["dft", "python"],
  // 경영지원
  ["fin", "tax"], ["fin", "erp"], ["tax", "erp"],
  ["fin", "cost"], ["cost", "erp"], ["cost", "procure"],
  ["hrm", "payroll"], ["payroll", "labor"], ["hrm", "labor"],
  ["hrm", "recruit"], ["recruit", "hrd"], ["hrd", "hrm"],
  ["legal", "labor"], ["legal", "procure"], ["fin", "procure"],
  ["procure", "scm"], ["scm", "erp"], ["security", "erp"],
  ["security", "legal"],
  // 도메인 간 다리: 경영 데이터도 분석 대상 (온톨로지의 핵심 메시지)
  ["fin", "python"], ["hrm", "python"], ["erp", "bigdata"],
  ["scm", "anomaly"], ["hrd", "ml"], ["cost", "sql"],
];
...
const TEAM_SKILL_POOL = {
  회로설계팀: ["verilog", "verification", "analog", "layout", "lowpower", "dft", "pkg", "python"],
  공정기술팀: ["litho", "etch", "thinfilm", "implant", "cmp", "metrology", "equip", "yield", "spc", "python"],
  품질보증팀: ["spc", "reliability", "fa", "eightd", "custqual", "atetest", "yield", "python"],
  DX데이터팀: ["python", "sql", "ml", "anomaly", "cv", "mlops", "bigdata", "spc"],
  재무회계팀: ["fin", "cost", "tax", "erp", "procure", "scm", "python"],
  경영지원팀: ["hrm", "recruit", "hrd", "payroll", "labor", "legal", "procure", "security", "erp"],
};
